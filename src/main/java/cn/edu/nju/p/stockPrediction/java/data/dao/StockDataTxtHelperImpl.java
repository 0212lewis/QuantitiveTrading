 ...
 */
public class StockDataTxtHelperImpl {

    private static final StockDataTxtHelperImpl INSTANCE = new StockDataTxtHelperImpl();

    private StockDataTxtHelperImpl(){
    ...
    public Map<String,Map<String, StockPO>> getStockData(String year) {
        ...
        Map<String, StockPO> inMap = new HashMap<>();
        try {
            InputStreamReader reader = new InputStreamReader(Thread.currentThread().getContextClassLoader().getResourceAsStream("data/2017 .csv"), "gbk");
...
//                    String quoteChange = df.format(Double.parseDouble(data[9])); //涨跌幅
                    ...
                    StockPO po = new StockPO(date, open, high, low, close, volume, close, code, name, "SZ", "test", isOpen,lastClose);
                    inMap.put(date, po);
                }else{
...
//                    String quoteChange = df.format(Double.parseDouble(data[9])); //涨跌幅
                    boolean isOpen = Integer.parseInt(data[10]) == 1; //是否开盘
                    StockPO po2 = new StockPO(date, open, high, low, close, volume, close, code, name, "SZ", "", isOpen,lastClose);
                    inMap.put(date, po2);
                }
                str = br.readLine();
            }
